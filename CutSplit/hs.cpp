...
hstrie::hstrie(int number, pc_rule *subset, int binth1, hs_node_t *node)
{
  ...
  LookupHSTree(&ruleset, node);
	...
	if (binth != 1)
	{
		//   result.total_mem_kb += (double)(gLeafTotalRule * 20)/1024; // Each rule need a store of (32+5)+(32+5)+(16+16)+(16+16)+(8+8) = 154b = 19.25B
		// result.total_mem_kb += (double)(ruleset.num * 20)/1024;
		result.total_mem_kb += 0;
	}
...
/*
 * Trace the leaf nodes to see the replication condition
 */
void hstrie::trace_leaf(hs_node_t *rootnode)
{
	// Record the leaf nodes
	printf("=== Start to trace the HS Tree ===\n");
	...
	int leaf_num = 0;
	int rule_num = 0;
	vector<hs_node_t *> nodeVec;
	nodeVec.push_back(node);
	map<unsigned int, unsigned int> m;
	while (!nodeVec.empty())
	{
		hs_node_t *tmpNode = nodeVec.back();
		nodeVec.pop_back();
		if (tmpNode->child[0] != NULL)
		{
			nodeVec.push_back(tmpNode->child[0]);
			nodeVec.push_back(tmpNode->child[1]);
		}
		else if (tmpNode != NULL)
		{
			leaf_num++;
			rule_num += tmpNode->ruleset->num;
			unsigned int ruleID_tmp = 0xFFFFFF;
			for (i = 0; i < tmpNode->ruleset->num; i++)
			{
				if (ruleID_tmp > tmpNode->ruleset->ruleList[i].pri)
				{
					ruleID_tmp = tmpNode->ruleset->ruleList[i].pri;
				}
			}
			if (m.find(ruleID_tmp) == m.end())
			{
				m[ruleID_tmp] = 1;
			}
			else
			{
				m[ruleID_tmp] = 1 + m[ruleID_tmp];
			}
		}
	}

	printf("Leaf: %d; Rule: %d\n", leaf_num, rule_num);
...
int hstrie::BuildHSTree(rule_set_t *ruleset, hs_node_t *currNode, unsigned int depth)
{
	...
	if (ruleset->num <= binth)
	{
		...
		assert(currNode->ruleset->num <= binth);
		gChildCount++;
		gNumLeafNode++;
		gLeafTotalRule += currNode->ruleset->num;
	...
	for (dim = 0; dim < DIM; dim++)
	{
		...
		if (pos >= 3)
		{
			...
			for (i = 0; i < pos - 1; i++)
			{
				...
				{
					if (ruleset->ruleList[j].range[dim][0] <= segPoints[dim][i] && ruleset->ruleList[j].range[dim][1] >= segPoints[dim][i + 1])
					{
						hightList[i]++;
						hightAll++;
					}
			...
			{ /* possible choice for d2s, pos-1 is the number of segs */
				float hightSum = 0;
	...
	for (num = 0; num < ruleset->num; num++)
	{
		if (ruleset->ruleList[num].range[d2s][0] <= range[0][1] && ruleset->ruleList[num].range[d2s][1] >= range[0][0])
		{
	...
	for (num = 0; num < ruleset->num; num++)
	{ // low<=end && high>=start
		if (ruleset->ruleList[num].range[d2s][0] <= range[1][1] && ruleset->ruleList[num].range[d2s][1] >= range[1][0])
		{
			tempRuleNumList[pos] = num;
			pos++;
		}
